...
namespace KDScorpionEngine.Content
{
    ...
    internal static class AtlasManager
    {
        ...
        /// <returns>True is the frame name is an animating frame.</returns>
        internal static bool IsAnimatingFrame(string frameName)
            => frameName.Contains('[', StringComparison.OrdinalIgnoreCase) && frameName.Contains('-', StringComparison.OrdinalIgnoreCase) && frameName.Contains(']', StringComparison.OrdinalIgnoreCase);

        /// <summary>
        /// Extracts the sub texture name from a formatted animating sub texture frame name.
        /// </summary>
        /// <param name="frameName">The name of the entire frame.</param>
        /// <returns>The sub stexture ID extracted from the name.</returns>
        ...
        private static void CheckData(AtlasData data)
        {
             ...
             *
             * ATLAS RULES
             * 1. Each individual sub texture name may or may not have meta data structure symbols embedded into the name itself
             ...
             * 3. The right bracket must be to the right of the left bracket and the last character of the entire name
             * 3. The dash must be between the 2 brackets
             * 4. The meta data starts at the left bracket and ends at the right bracket
             * 5. There must be data in between the left bracket and the dash and the right bracket and the dash
             ...
             * 8. The data to the right of the dash must not have any duplicate numbers that reside in the same group of animation sub textures with the same sub texture ID.
             */

            var frameNameList = data.FrameNameList;
            var items = new List<string>();

            // Check For Single Name Structure
            ...
            // Animating Sub Texture Frame Index Sequence Checking
